#!/usr/bin/env bash
# Build "Local Dictation.app" with py2app (alias mode).
#
...
# real native bootloader as the bundle's main executable, so TCC sees "Local
# Dictation" and the mic prompt works. Alias mode references this project/venv in
...
echo "Building Local Dictation.app…"
rm -rf build dist
# Remove the obsolete shell-wrapper bundle if present.
rm -rf "Local Dictation.app"

.venv/bin/python setup.py py2app -A >/tmp/local-dictation-py2app.log 2>&1 \
  || { echo "Build failed — see /tmp/local-dictation-py2app.log"; tail -20 /tmp/local-dictation-py2app.log; exit 1; }

APP="${PROJECT_DIR}/dist/Local Dictation.app"
echo
echo "✅ Built: ${APP}"
echo
echo "Next:"
...
echo "  3. If the fn key does nothing, add 'Local Dictation' under System Settings"
echo "     → Privacy & Security → Accessibility, then relaunch."
echo "  4. Hold fn, speak, release — text lands at your cursor."
echo
echo "Tip: drag dist/Local Dictation.app to /Applications, and add it to System"
echo "     Settings → General → Login Items to launch at startup."
echo "Logs: ~/Library/Logs/LocalDictation.log"
